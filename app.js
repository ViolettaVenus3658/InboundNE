...
const _AUTH = {
    // SHA-256 de "luft"
    UH: '08936e53cd8b59d262e7cd0f95268dd438612961a469f2f1ed8837e59dedd968',
    // SHA-256 de "luft@123"
    PH: '12e06fd9e0fccc130b3cef993d0e0fbf94bc54054446bcbd43f296906d9a68a4',
    SESSION_KEY: 'inbound_auth_v1',
    _failCount: 0,
    _locked: false
};
...
// Verificação imediata: se sessão válida, libera o app
(function initAuth() {
    if (sessionStorage.getItem(_AUTH.SESSION_KEY) === '1') {
...
window.doLogin = async function(e) {
    ...
    const user = String(document.getElementById('login-username').value).trim().toLowerCase().slice(0, 50);
    const pass = String(document.getElementById('login-password').value).slice(0, 100);

    // Delay progressivo para dificultar força bruta
    await new Promise(r => setTimeout(r, 400 + (_AUTH._failCount * 800)));

    const [uHash, pHash] = await Promise.all([_sha256(user), _sha256(pass)]);

    if (uHash === _AUTH.UH && pHash === _AUTH.PH) {
        _AUTH._failCount = 0;
        sessionStorage.setItem(_AUTH.SESSION_KEY, '1');
        document.getElementById('login-screen').style.display = 'none';
        document.getElementById('main-app').style.display = '';
    } else {
        _AUTH._failCount++;
        if (_AUTH._failCount >= 5) {
            _AUTH._locked = true;
            errDiv.textContent = 'Muitas tentativas. Atualize a página para tentar novamente.';
        } else {
...
window.doLogout = function() {
    sessionStorage.removeItem(_AUTH.SESSION_KEY);
...
function setMobileNavState(forceOpen) {
    const sidebar = document.getElementById('sidebar-nav');
    const toggleButton = document.getElementById('mobile-nav-toggle');
    if (!sidebar || !toggleButton) return;

    const isMobile = mobileNavQuery.matches;
    const isOpen = isMobile && Boolean(forceOpen);
...
mobileNavQuery.addEventListener('change', () => {
    if (!mobileNavQuery.matches) setMobileNavState(false);
});
...
async function postToGoogleScript(payload, operationName) {
    ...
    if (!response.ok) {
        throw new Error(`Falha no envio (${operationName}): HTTP ${response.status}`);
    }

    const finalUrl = String(response.url || '');
    if (finalUrl.includes('accounts.google.com') || finalUrl.includes('/signin/')) {
        throw new Error('Endpoint Google Script requer autenticação (deploy não público).');
    }
...
window.sincronizarGoogle = async function() {
    ...
    try {
        ...
        if(linhasGestao.length > 0) {
            await postToGoogleScript({ tipo: 'visao_geral', linhas: linhasGestao }, 'visao_geral');
        }

        // 2. Enviar Itens
        if(linhasItens.length > 0) {
            await postToGoogleScript({ tipo: 'itens', linhas: linhasItens }, 'itens');
        }
        ...
        if (String(err.message || '').includes('autenticação') || String(err.message || '').includes('HTTP 401')) {
            showToast('Revise o deploy do Apps Script: acesso deve estar em "Qualquer pessoa".', 'error');
        }
    } finally {
